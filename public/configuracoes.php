...
$page_title = __('nav.settings') . " - " . __('site.title');
$page_description = __('settings.description');
require_once '../includes/header.php';
?>

<main style="padding: 2rem 1rem; min-height: 70vh;">
    <div style="max-width: 800px; margin: 0 auto;">
        <h1 style="color: #5e2b2b; margin-bottom: 2rem; text-align: center;">⚙️ <?php echo __('settings.title'); ?></h1>
        
        <div style="background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <form id="configForm">
                <!-- Tema -->
                <div class="config-section">
                    <h3>🎨 <?php echo __('settings.appearance'); ?></h3>
                    
                    <div class="config-item">
                        <label><?php echo __('settings.theme'); ?>:</label>
                        <select name="tema" id="tema">
                            <option value="claro" <?= $configuracoes['tema'] == 'claro' ? 'selected' : '' ?>><?php echo __('settings.light'); ?></option>
                            <option value="escuro" <?= $configuracoes['tema'] == 'escuro' ? 'selected' : '' ?>><?php echo __('settings.dark'); ?></option>
                        </select>
                    </div>
                    
                    <div class="config-item">
                        <label><?php echo __('settings.primary_color'); ?>:</label>
                        <input type="color" name="cor_primaria" value="<?= $configuracoes['cor_primaria'] ?>">
                    </div>
                    
                    <div class="config-item">
                        <label><?php echo __('settings.font_size'); ?>:</label>
                        <select name="tamanho_fonte">
                            <option value="pequeno" <?= $configuracoes['tamanho_fonte'] == 'pequeno' ? 'selected' : '' ?>><?php echo __('settings.small'); ?></option>
                            <option value="medio" <?= $configuracoes['tamanho_fonte'] == 'medio' ? 'selected' : '' ?>><?php echo __('settings.medium'); ?></option>
                            <option value="grande" <?= $configuracoes['tamanho_fonte'] == 'grande' ? 'selected' : '' ?>><?php echo __('settings.large'); ?></option>
                        </select>
                    </div>
                </div>
                ...
                <!-- Layout -->
                <div class="config-section">
                    <h3>📱 <?php echo __('settings.layout'); ?></h3>
                    
                    <div class="config-item">
                        <label><?php echo __('settings.product_view'); ?>:</label>
                        <select name="layout">
                            <option value="grid" <?= $configuracoes['layout'] == 'grid' ? 'selected' : '' ?>><?php echo __('settings.grid'); ?></option>
                            <option value="lista" <?= $configuracoes['layout'] == 'lista' ? 'selected' : '' ?>><?php echo __('settings.list'); ?></option>
                        </select>
                    </div>
                    
                    <div class="config-item">
                        <label><?php echo __('settings.products_per_page'); ?>:</label>
                ...
                <div class="config-section">
                    ...
                    <div class="config-item">
                        <label>
                            ...
                            <?php echo __('settings.receive_notifications'); ?>
                        </label>
                    </div>
                </div>
        ...
        <div id="preview" style="background: white; padding: 2rem; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); margin-top: 2rem;">
            <h3 style="margin-bottom: 1rem;">👁️ <?php echo __('settings.preview'); ?></h3>
            <div class="preview-content">
                <div class="produto-card-preview">
                    ...
                    <h4><?php echo __('settings.sample_product'); ?></h4>
