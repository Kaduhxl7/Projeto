...
class Notification {
    ...
    public function getByUser($id_usuario, $limit = 20) {
        ...
        $sql = "SELECT n.*, 
                ...
                LEFT JOIN brechos b ON n.brecho_id = b.id
                WHERE n.id_usuario = ?
    ...
    public function notifyNewProduct($produto_id, $produto_nome, $brecho_id = null) {
        $titulo = __('notifications.new_product_title');
        $mensagem = __('notifications.new_product_message', ['produto' => $produto_nome]);
    ...
    public function notifyPromotion($titulo_promocao, $descricao, $brecho_id = null, $brecho_nome = null) {
        $titulo = __('notifications.promotion_title', ['brecho' => $brecho_nome ?? 'um brechó']);
        $mensagem = __('notifications.promotion_message', ['promocao' => $titulo_promocao, 'descricao' => $descricao]);
    ...
    public function notifyStoreUpdate($brecho_id, $brecho_nome) {
        $titulo = __('notifications.store_update_title', ['brecho' => $brecho_nome]);
        $mensagem = __('notifications.store_update_message', ['brecho' => $brecho_nome]);
