...
class NotificationObserver implements Observer {
     ...
     */
    private function createNotificationForAllUsers(string $titulo, string $mensagem, string $tipo, ?int $produtoId = null): int {
        try {
            ...
            $sql = "INSERT INTO notificacoes (id_usuario, titulo, mensagem, tipo, produto_id) VALUES {$placeholders}";
            $stmt = $this->db->prepare($sql);
     ...
     */
    public function createNotificationForUser(int $userId, string $titulo, string $mensagem, string $tipo, ?int $produtoId = null): bool {
        try {
            $sql = "INSERT INTO notificacoes (id_usuario, titulo, mensagem, tipo, produto_id) VALUES (?, ?, ?, ?, ?)";
            $stmt = $this->db->prepare($sql);
            return $stmt->execute([$userId, $titulo, $mensagem, $tipo, $produtoId]);
        } catch (Exception $e) {
